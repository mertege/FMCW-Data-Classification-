...
% end
%%
    doppler = linspace(-8000,8000,size(spectrogram_fast_resized,1));    

time=linspace(0,16,size(spectrogram_fast_resized,2));
T=time;
F=doppler;

k = 1;
figure(4)
imagesc(T,F,spectrogram_fast_resized(:,:,k))
xlabel('Time(s)');
ylabel('Frequency(Hz)')
...
figure(5)
k = 2;
imagesc(T,F,spectrogram_slow_resized(:,:,k))
xlabel('Time(s)');
ylabel('Frequency(Hz)')
